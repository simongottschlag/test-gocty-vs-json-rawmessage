...
func ctyCheck(want, got map[string]interface{}) error {
	...
	for _, key := range keys {
		...
		wantV, gotV, err := getCtyValues(want, got)
		if err != nil {
			return err
		}

		err = isCtyValueValid(wantV, gotV)
		if err != nil {
			return fmt.Errorf("claim %q not valid: %w", key, err)
		}
	}
